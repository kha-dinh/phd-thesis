...
We also discuss the possible mitigation for brute-forcing attacks, and optimizations, at the end of the section.
...
Our evaluation (\cref{sec:evaluation}) indicates that most of \thename's overhead arises from the per-load and per-store pointer authentication mechanism, which requires executing \gls{pa} on every memory load and store.
A promising strategy to reduce this overhead and enable practical adoption is to minimize register spilling of pointers, thereby reducing the number of pointer authentication operations.
To achieve this optimization, the compiler's register allocation and spilling logic must distinguish between pointer types and non-pointer (e.g., integer) values when deciding which registers to spill to memory.
Implementing this strategy requires pointer-awareness across multiple compiler backend passes.
While challenging, this approach is already being pursued by compiler developers.
For example, LLVM 22.0~\footnote{\url{https://llvm.org/docs/PointerAuth.html}} provides not only built-in intrinsics for pointer authentication, but also \emph{function attributes} to annotate pointer authentication operations on functions and \emph{operand bundles} to prevent spilling of selected registers across function calls.
We expect that these efforts will also aid in reducing the overhead of \thename.
