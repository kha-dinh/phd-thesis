...
\hdr{Defining isolation boundaries}
...
Another approach to specify the isolation boundary between domains is to leverage a separate specification, much like how SELinux~\cite{selinux} and AppArmor~\cite{apparmor-paper} manage security profiles per applications.
...
Such an attack surface must be eliminated or minimized through validation and interface narrowing when porting existing programs to use \thename.
An approach to detect these interface vulnerabilities is to employ software testing techniques like fuzzing~\cite{civ} or symbolic execution~\cite{mansour2025rakis} to detect potential security violations in the boundaries.
If a program is redesigned or written from scratch with \thename, then a conscious effort can be made to have a secure interface, e.g., by applying secure interface design guidelines~\cite{civ}.
...
\hdr{System call filtering}
\thename enforces a coherent in-process isolation scheme by authenticating system resource handles, such as file paths and file descriptors.
Prior work has long recognized the system call interface as a critical security boundary, as it provides a powerful avenue for privilege escalation (e.g., by executing arbitrary binaries).
Consequently, a common defense is to restrict the set of system calls and system resources that a process is permitted to access.

Previous work has sought to restrict system calls at the process-level.
Linux Seccomp is one of the most widely deployed mechanisms for this and is a security staple in execution environments ranging from Android applications and \texttt{systemd} services to Docker containers~\cite{confine}, Amazon Firecracker~\cite{firecracker}, and Kubernetes~\cite{kubernetes-seccomp}.
SELinux~\cite{selinux}, based on Linux Security Module (LSM) further constrains the system call interface through a Mandatory Access Control (MAC) framework, assigning fine-grained security labels to system resources and enforcing strict information-flow policies.
Another line of work is system call specialization, which tries to automatically construct policies that minimize the set of \gls{syscall} an application is allowed to call using configuration file-aware~\cite{c2c}  or application phases-aware~\cite{ghavamnia2020temporal} analyses.
Despite their effectiveness, these mechanisms and policies primarily operate at process granularity; finer-grained access control within a process remains largely unsupported.
% However, emerging software architectures increasingly require isolation between mutually distrusting in-process domains.


Recent research has explored system call filtering mechanisms better suited for in-process isolation~\cite{donky,jenny,cerberus,muswitch}.
Jenny~\cite{jenny} proposes a secure and efficient system call reference monitor that delegates access control decisions to userspace.
$\mu$Switch~\cite{muswitch} leverages implicit context switches to defer kernel resource context switching until a system call is issued, improving performance.
While these approaches rely on reference monitors and require developers to explicitly specify filtering rules, \thename integrates cryptographically secured \fd references directly into the program semantics.
Building on this foundation, \thename relies on \gls{pa}-assisted authentication of system resources, enabling efficient and coherent protection across resource life cycles without external policy specifications.
