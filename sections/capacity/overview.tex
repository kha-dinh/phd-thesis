...
\autoref{fig:overview} illustrates an overview of \thename.
...
Consider the common pattern of sensitive object use shown in \autoref{fig:overview} (Domain B).
...
\begin{figure}[t]
	...
	\caption{\thename's reference encapsulation of pointers and file descriptors with cryptographic authentication code (AC).}
	\label{fig:ref-encapsulation}
\end{figure}

\hdr{Object encapsulation}
...
\begin{listing}[t]
	\input{figures/capacity/capacity-code-example}
...
\begin{table*}[t]
  ...
  \cline{2-3}
   & \texttt{DOM\_PRIV}                                             & Marks a \emph{source} domain-private pointer for taint analysis.\\
		...
		\bottomrule
	\end{tabularx}
	\caption{\thename APIs and program annotations}
	\label{tab:apis}
\end{table*}
