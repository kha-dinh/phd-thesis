...
To bolster customers' trust in cloud, cloud providers are increasingly adopting specialized hardware from compute vendors (e.g., Intel, AMD).
...
\gls{cvm} brings unprecedented compatibility for protecting cloud workloads, making it feasible do deploy existing applications without modification, and cutting-edge applications like AI inference~\cite{yifan2025pipellm}, inside \glspl{tee}.
...
The \gls{cvm} model now places the confidential application, along with all of its supporting software, within the trust boundary.
However, doing so places a significant amount of code inside the TEE \dash\ specifically, the \gls{os} must now be a part of the \gls{tcb}, which contain tens of millions of lines of code and thousands of known vulnerabilities.

This increased code size increases the attack surface.
